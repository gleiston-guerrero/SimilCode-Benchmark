...
- Salidas archivadas: `out_jplag_cs/results.csv` y `out_jplag_java/results.csv` (2 775 comparaciones cada uno), más los contenedores `out_jplag_cs.jplag` y `out_jplag_java.jplag`. Logs completos en `jplag_cs.log` y `jplag_java.log`.
...
## 7. Higiene de credenciales y del repositorio

**Incidente de exposición.** El archivo `ejecutar_benchmark.ps1`, que asignaba las cuatro claves de API en texto plano, fue versionado en el commit `8d22d98` y publicado en un repositorio **público** de GitHub. Se detectó al auditar el historial antes de la sincronización del 1 de agosto de 2026. Dos de las cuatro credenciales coincidieron con los patrones de detección habituales; se asumen comprometidas las cuatro, porque los formatos de DeepSeek y Google no siempre son detectables por expresión regular.

**Acciones ejecutadas.** Reescritura del historial con `git filter-repo --path ejecutar_benchmark.ps1 --invert-paths`, seguida de `push --force` sobre ramas y etiquetas. El clon de la segunda máquina se realineó con `fetch` + `reset --hard` —nunca con `--allow-unrelated-histories`, que habría reintroducido el commit comprometido— y se purgaron sus objetos locales con `reflog expire` y `gc --prune=now`. Verificado en ambos equipos: `git log --all -- ejecutar_benchmark.ps1` no devuelve resultados.

**Limitación reconocida.** La reescritura del historial no revierte la exposición. Un commit permanece accesible por su identificador en GitHub hasta la recolección de basura del servidor, y las bifurcaciones ajenas no se limpian. La rotación de credenciales es la única medida que elimina el riesgo; la limpieza del historial es complementaria.

**[COMPLETAR: fecha de rotación efectiva de las cuatro claves y resultado de la revisión de consumo anómalo en cada proveedor.]**

**Identificador de Moss.** El script `moss` contiene un identificador que autentica las consultas ante Stanford. Nunca llegó a versionarse —verificado: `git log --all -- moss` no devuelve resultados—, pero apareció en una conversación asistida, por lo que se solicita su reposición a `moss-request@cs.stanford.edu`. **[COMPLETAR: fecha de solicitud y de reposición.]**

**Configuración preventiva.** Las credenciales se definen exclusivamente como variables de entorno, nunca dentro de archivos del árbol de trabajo. El `.gitignore` excluye `moss`, `ejecutar_benchmark.ps1`, `*.key`, `.env`, la carpeta local `No Commit/`, los binarios de terceros (`*.jar`) y las claves de cegamiento de la evaluación en curso.

**Anonimato para revisión doble ciego.** La URL del repositorio contiene el nombre del autor. Citarla en el manuscrito enviado a revisión desanonimiza al equipo y puede motivar rechazo administrativo. **Para el envío debe emplearse un espejo anónimo**, reservando la URL real para la versión final aceptada. **[COMPLETAR: URL anónima empleada.]**

**Datos personales de terceros.** La columna `evaluator` de `metadata.csv` contenía el nombre real del segundo evaluador en las 120 filas. Se sustituyó por el identificador `EVAL_01` antes de la publicación; la versión con nombres se conserva únicamente en local.
...
3. Rotar las cuatro claves de API y revisar el consumo de cada proveedor (ver sección 7).
4. Reejecutar la validación Big O contra la API oficial, **con el modelo que integre SimilCode en producción**, no con otro.
5. Anclaje humano de la calidad de la retroalimentación: cuatro evaluadores ciegos, escala continua 0–100, sobre los 32 ítems que ya calificó el panel automático. Analizar con ICC(2,1) e ICC(2,k) y contrastar con el panel.
6. Decidir el modelo integrado en producción a la luz del benchmark y del anclaje humano.
7. Regenerar todas las figuras desde los resultados nuevos, con el texto en inglés.
8. Reescribir Resultados, Discusión y Conclusiones con los cuatro modelos actuales.
9. Preparar el espejo anónimo del repositorio para el envío a revisión doble ciego.
